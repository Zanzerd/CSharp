    ...
    public class LevenshteinCalculator_Tests
    {
        ...
        public void ReturnAllPairwiseComparisons()
        {
            ...
            var expected =
                ...
                {
                    new ComparisonResult(tokenizedDocuments[0], tokenizedDocuments[1], 1),
                    new ComparisonResult(tokenizedDocuments[0], tokenizedDocuments[2], 2),
                    new ComparisonResult(tokenizedDocuments[1], tokenizedDocuments[2], 1),
                };
            var actual = levenshteinCalculator.CompareDocumentsPairwise(tokenizedDocuments);
            // Сравнение коллекций без учета порядка:
